...
搬到 windows 下后立刻出了问题。`import mediapipe` 的时候提示动态链接失败了。查了一下，发现是[比较新的 mediapipe 的问题](https://github.com/google-ai-edge/mediapipe/issues/6029)，降级到 0.10.14 的版本后就能正常跑通了。
...
发给有 macbook 的学长测试了一下，结果发现跑不起来，查阅问题发现是 [metal 只接受带有 alpha 通道的颜色格式](https://github.com/google-ai-edge/mediapipe/issues/5265)，导致 gpu 加速的时候出现了图像帧格式不兼容的问题，遂加上颜色格式特判（`SRGB -> SRGBA`），然后就能正常跑起来了。
